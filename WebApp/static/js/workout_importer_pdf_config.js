// Alpine entry for PDF workout import (async + polling).
function workoutImporterPdf() {
  return window.createWorkoutImporter({
    sourceType: 'pdf',
    accept: '.pdf',
    extPattern: /\.pdf$/i,
    maxBytes: 20 * 1024 * 1024,
    submitUrl: '/api/allenamenti/import/pdf/',
    statusUrl: '/api/allenamenti/import/pdf/status/',
    async: true,
    pollIntervalMs: 1500,
    showSourceBadge: true,
    steps: [
      { icon: 'ph ph-file-pdf', label: 'Analisi del PDF...' },
      { icon: 'ph ph-magnifying-glass', label: 'Identificazione pagine rilevanti...' },
      { icon: 'ph ph-scan', label: 'OCR delle pagine necessarie...' },
      { icon: 'ph ph-brain', label: 'Estrazione AI della scheda...' },
      { icon: 'ph ph-link', label: 'Match esercizi nel database...' },
    ],
    motivationalMessages: [
      'Sto leggendo il documento pagina per pagina...',
      'Sto identificando le sezioni di allenamento...',
      'Sto leggendo le pagine scansionate...',
      'Sto estraendo sets, reps, carichi e recuperi...',
      'Sto associando gli esercizi al database...',
    ],
    phaseMap: { analyze: 0, classify: 1, ocr: 2, extract: 3, finalize: 4 },
    minPhaseMs: [10000, 10000, 3000, 4000, 1500],
    copy: {
      formatError: 'Formato non supportato (solo .pdf)',
      tooLargeError: 'File troppo grande (max 20 MB)',
      invalidFileError: 'Non siamo riusciti a leggere questo PDF. Prova con un file diverso.',
    },
  });
}
window.workoutImporterPdf = workoutImporterPdf;
